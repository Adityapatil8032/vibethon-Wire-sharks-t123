        ...
        <div className="bg-sleek-card border border-sleek-border rounded-3xl p-8 md:p-12 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative w-full max-w-2xl mx-auto aspect-video bg-black/60 rounded-2xl border border-sleek-border mb-10 overflow-hidden shadow-inner">
            ...
            <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible p-6 relative z-10">
              ...
              {points.map((p, i) => {
                const predictedY = p.x * weight + bias;
                return (
                  <motion.line
                    key={`error-${i}`}
                    x1={p.x * 10}
                    ...
                    strokeWidth="1"
                    strokeDasharray="2 2"
                    className="transition-all duration-300 ease-out opacity-70"
                  />
                );
              })}
          ...
          <div className="flex items-center justify-center space-x-4 md:space-x-12 mb-16">
            ...
            <div className="flex flex-col justify-around h-64 relative">
              ...
              <svg className="absolute inset-0 w-full h-full -z-10 overflow-visible" style={{ left: '-50%', width: '200%' }}>
                ...
                {w1 !== 0 && (
                  <motion.line 
                    x1="0" y1="16%" x2="100%" y2="50%" 
                    stroke="#06b6d4" 
                    strokeWidth={Math.abs(w1)} 
                    strokeDasharray="10 10"
                    initial={{ strokeDashoffset: 0 }}
                    animate={{ strokeDashoffset: w1 > 0 ? -40 : 40 }}
                    transition={{ repeat: Infinity, duration: 2 / Math.abs(w1), ease: "linear" }}
                ...
                {w2 !== 0 && (
                  <motion.line 
                    ...
                    stroke="#06b6d4" 
                    strokeWidth={Math.abs(w2)} 
                    strokeDasharray="10 10"
                    initial={{ strokeDashoffset: 0 }}
                    animate={{ strokeDashoffset: w2 > 0 ? -40 : 40 }}
                    transition={{ repeat: Infinity, duration: 2 / Math.abs(w2), ease: "linear" }}
                    className="opacity-60"
                  />
                )}
                {w3 !== 0 && (
                  <motion.line 
                    x1="0" y1="84%" x2="100%" y2="50%" 
                    stroke="#06b6d4" 
                    strokeWidth={Math.abs(w3)} 
                    strokeDasharray="10 10"
                    initial={{ strokeDashoffset: 0 }}
                    animate={{ strokeDashoffset: w3 > 0 ? -40 : 40 }}
                    transition={{ repeat: Infinity, duration: 2 / Math.abs(w3), ease: "linear" }}
                    className="opacity-60"
                  />
                )}
              </svg>
